...
const SHEETS = [
  ...
  { frontType: 'step', backType: 'step', frontData: JOURNEY_STEPS[6], backData: JOURNEY_STEPS[7] },
  {
    ...
    backType: 'backcover',
    frontData: {},
    backData: { title: 'Complete Wellness', desc: 'Your path to harmony and balance is ongoing. We stand by you every step of the way.' }
  }
];

// ── Scroll units ──────────────────────────────────────────────────────────────
// INTRO_UNITS: scroll space for the title-fade + book-scale-up animation
// FLIPS: one unit per sheet flip
// Total progress: 0 → INTRO_UNITS + FLIPS
const FLIPS        = SHEETS.length;   // 6
const INTRO_UNITS  = 1.5;
const OUTRO_UNITS  = 1.5;             // scroll space to scale book back down
...
export default function PatientJourneyPage() {
  ...
  // Background overlay: darkens simultaneously with zoom in, fades during zoom out
  const overlayOpacity = scrollProgress <= INTRO_UNITS
    ...
    <>
      ...
      >
        ...
        <div className="sticky top-0 h-screen w-full flex items-center justify-center overflow-hidden pt-[76px]">
          ...
          {/* ── Outro text — top-centre, zooms in ── */}
          <div
            className="absolute z-20 text-center pointer-events-none select-none px-6"
            style={{
              top: '12%',
          ...
          >
            ...
            <h2 className="font-display text-[clamp(2.4rem,5vw,4rem)] text-[#3b4a38] leading-tight mb-4" style={{ fontWeight: 400 }}>A Restored Balance</h2>
            <p className="text-[#556353] font-body text-sm md:text-base leading-relaxed max-w-[38ch] mx-auto">
          ...
          {/* ── Book container — height avoids navbar; width constrained ── */}
          <div
          ...
          >
            {/* Spine shadow */}
            <div className="absolute top-0 bottom-0 left-1/2 w-20 -ml-10 bg-gradient-to-r from-transparent via-black/15 to-transparent z-50 pointer-events-none" />
            ...
            {SHEETS.map((sheet, idx) => {
              ...
              // True 3D volume parameters
              const T = isCoverSheet ? 16 : 8; // Thickness in pixels
              ...
              return (
                ...
                >
                  ...
                  {/* Right Edge (Solid 3D Wall) */}
                  <div 
                    className="absolute top-0 bottom-0"
                    style={{ right: `-${T/2}px`, width: `${T}px`, background: edgeColor, transform: 'rotateY(90deg)' }} 
                  />
                  {/* Bottom Edge */}
                  <div 
                    className="absolute left-0 right-0"
                    style={{ bottom: `-${T/2}px`, height: `${T}px`, background: edgeColor, transform: 'rotateX(-90deg)' }} 
                  />
                  {/* Top Edge */}
                  <div 
                    className="absolute left-0 right-0"
                    style={{ top: `-${T/2}px`, height: `${T}px`, background: edgeColor, transform: 'rotateX(90deg)' }} 
                  />
                </div>
              );
            })}
...
function RenderPage({ type, data, side }: { type: string; data: any; side: 'left' | 'right' }) {
  ...
  if (type === 'cover') {
    return (
      <div className="w-full h-full flex flex-col items-center justify-center p-8 md:p-16 text-center bg-[#2b4230] relative border-l border-black/20" style={{ boxShadow: 'inset 0 0 100px rgba(0,0,0,0.5)' }}>
        <div className="absolute top-0 bottom-0 left-[2%] w-4 bg-gradient-to-r from-black/60 via-transparent to-black/30" />
  ...
  if (type === 'backcover') {
    return (
      <div className="w-full h-full flex flex-col items-center justify-center p-8 md:p-16 text-center bg-[#2b4230] relative border-r border-black/20" style={{ boxShadow: 'inset 0 0 100px rgba(0,0,0,0.5)' }}>
        <div className="absolute top-0 bottom-0 right-[2%] w-4 bg-gradient-to-l from-black/60 via-transparent to-black/30" />
        <div className="absolute inset-4 md:inset-8 border-2 border-[#a8843e]/40 rounded-lg pointer-events-none" />
  ...
  const step = data;
  return (
    <div className={`p-8 md:p-16 h-full flex flex-col relative w-full ${isLeft ? 'items-end text-right' : 'items-start text-left'}`} style={paperTexture}>
      <div
